...
// TestJobController verifies that our ConfigMap behavior works.
	...
	for i := 0; i < 3; i++ {
		t.Run(fmt.Sprintf("attempt-%d", i), func(t *testing.T) {
			...
			for i := 0; i < 3; i++ {
				...
				numJobs := rand.Intn(10)
				for i := 0; i < numJobs; i++ {
					if err := c.Create(context.TODO(), &datav1alpha1.Job{
						ObjectMeta: metav1.ObjectMeta{Namespace: namespaceName, Name: fmt.Sprintf("Job-%d", i)},
						Spec:       datav1alpha1.JobSpec{Script: fmt.Sprintf("intended-%d", i)},
					}); err != nil {
			...
			t.Logf("waiting for all Jobs in cluster %s to have a correct status", workspaceCluster)
			var allJobs datav1alpha1.JobList
			if err := wait.PollImmediate(100*time.Millisecond, wait.ForeverTestTimeout, func() (done bool, err error) {
				fetchErr := c.List(context.TODO(), &allJobs)
				if fetchErr != nil {
					t.Logf("failed to get Jobs in cluster %s: %v", workspaceCluster, err)
					return false, fetchErr
				}
				var errs []error
				for _, Job := range allJobs.Items {
					if actual, expected := Job.Status.ID, totalJobs; actual != expected {
						errs = append(errs, fmt.Errorf("Job %s|%s .status.total incorrect: %d != %d", workspaceCluster, Job.Name, actual, expected))
					}
				}
				validationErr := errors.NewAggregate(errs)
				if validationErr != nil {
					t.Logf("Jobs in cluster %s invalid: %v", workspaceCluster, validationErr)
				}
				return validationErr == nil, nil
			}); err != nil {
				t.Fatalf("Jobs in cluster %s never got correct statuses: %v", workspaceCluster, err)
			}
		})
	}
}
