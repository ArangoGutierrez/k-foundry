...
import (
	...
	"github.com/ArangoGutierrez/k-foundry/config/consumers"
	foundry "github.com/ArangoGutierrez/k-foundry/config/foundry"
	foundryResources "github.com/ArangoGutierrez/k-foundry/config/foundry/resources"
...
// - foundry/resources/* - a set of Widget resources for KCP to manage. Automatically generated by kcp apigen
...
var (
	...
	clusterName = logicalcluster.NewPath("root:foundry")
)
...
func main() {
	...
	restCopy = rest.CopyConfig(restConfig)
	restfoundry := rest.AddUserAgent(kcpclienthelper.SetCluster(restCopy, clusterName), "bootstrap-foundry")
	foundryClient, err := client.New(restfoundry, client.Options{})
	if err != nil {
		log.Error(err, "unable to create client")
		os.Exit(1)
	}

	err = foundry.Bootstrap(ctx, rootClient)
	if err != nil {
		log.Error(err, "failed to bootstrap foundry")
		os.Exit(1)
	}

	err = foundryResources.Bootstrap(ctx, foundryClient)
	if err != nil {
	...
	if err != nil {
		log.Error(err, "failed to bootstrap clusters")
		os.Exit(1)
	}
}
