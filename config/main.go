...
import (
	...
	"github.com/ArangoGutierrez/k-slurm/config/consumers"
	jobs "github.com/ArangoGutierrez/k-slurm/config/jobs"
	jobResources "github.com/ArangoGutierrez/k-slurm/config/jobs/resources"
...
// - jobs/resources/* - a set of Widget resources for KCP to manage. Automatically generated by kcp apigen
...
var (
	...
	clusterName = logicalcluster.NewPath("root:jobs")
)
...
func main() {
	...
	restCopy = rest.CopyConfig(restConfig)
	restjobs := rest.AddUserAgent(kcpclienthelper.SetCluster(restCopy, clusterName), "bootstrap-jobs")
	jobsClient, err := client.New(restjobs, client.Options{})
	if err != nil {
		log.Error(err, "unable to create client")
		os.Exit(1)
	}

	err = jobs.Bootstrap(ctx, rootClient)
	if err != nil {
		log.Error(err, "failed to bootstrap jobs")
		os.Exit(1)
	}

	err = jobResources.Bootstrap(ctx, jobsClient)
	if err != nil {
